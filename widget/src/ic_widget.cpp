...
void ICWidget::loadConfig()
{
    try
    {
        ...
        ui_->text_edit_log->append("Successfully loaded calibration configuration");
    }
...
void ICWidget::saveConfig()
{
  ...
  // Camera intrinsics
  node["intrinsics"] = camera_intrinsics_dialog_->widget->getConfig();

  // Transform guesses
  node["camera_mount_to_camera_guess"] = camera_transform_guess_dialog_->widget->getConfig();
  node["target_mount_to_target_guess"] = target_transform_guess_dialog_->widget->getConfig();
  ...
  node["target_finder"] = target_dialogs_.at(target_type)->widget->getConfig();
...
void ICWidget::loadTargetFinder()
{
    ...
    QString target_type = ui_->combo_box_target_finder->currentText();
    YAML::Node target_finder_config = target_dialogs_.at(target_type)->widget->getConfig();
...
void ICWidget::loadData()
{
    ...
    try
    {
        ...
        problem_->camera_mount_to_camera_guess = camera_transform_guess_dialog_->widget->getConfig().as<Eigen::Isometry3d>();
        problem_->target_mount_to_target_guess = target_transform_guess_dialog_->widget->getConfig().as<Eigen::Isometry3d>();
        problem_->intr = camera_intrinsics_dialog_->widget->getConfig().as<CameraIntrinsics>();
...
void ICWidget::drawImage(int row, int col)
{
    ...
    {
        ui_->imageWidget->setImage(data.value<QPixmap>());
        update();
    }
}
