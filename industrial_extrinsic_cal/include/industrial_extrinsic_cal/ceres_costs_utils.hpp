...
class LinkRangeCameraTargetReprjErrorPK
{
public:
  LinkRangeCameraTargetReprjErrorPK(double ob_x, double ob_y, double ob_z, Pose6d link_pose, Point3d point)
    : ox_(ob_x), oy_(ob_y), oz_(ob_z), link_pose_(link_pose), point_(point)
  {
    link_posei_ = link_pose_.getInverse();
  }
                  ...
                  T *residual) const
  {
    ...
    // calculate residual between camera_point and point cloud points
  ...
  /** Factory to hide the construction of the CostFunction object from */
  /** the client code. */
  static ceres::CostFunction *Create(const double o_x, const double o_y, const double o_z, Pose6d pose, Point3d pnt)

  {
    return (new ceres::AutoDiffCostFunction<LinkRangeCameraTargetReprjErrorPK, 3, 6, 6>(
        new LinkRangeCameraTargetReprjErrorPK(o_x, o_y, o_z, pose, pnt)));
  }
  double ox_;         /** observed x location of object in point cloud */
  double oy_;         /** observed y location of object in point cloud */
  double oz_;         /** observed z location of object in point cloud */
  Pose6d link_pose_;  /*!< transform from world to link coordinates */
  Pose6d link_posei_; /*!< transform from link to world coordinates */
  Point3d point_;     /*! location of point in target coordinates */
};
