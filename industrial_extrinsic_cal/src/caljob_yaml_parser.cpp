...
ObservationScene parseSingleScene(const Node &node, int scene_id, CeresBlocks &blocks)
{
  ...
  try
  {
    ...
    for (int i = 0; i < (int)observation_node.size(); i++)
    {
      ...
      Roi temp_roi;
      if (!parseString(observation_node[i], "camera", camera_name))
        ROS_ERROR("no camera_name in observation");
      if (!parseString(observation_node[i], "target", target_name))
        ROS_ERROR("no target in observation");
      if (!parseString(observation_node[i], "cost_type", cost_type_string))
        ROS_ERROR("no cost_type in observation");
