...
int ROSCameraObserver::getObservations(CameraObservations &cam_obs)
{
  ...
  switch (pattern_)
  {
    ...
    {  // contain the scope of automatic variables
      ...
      if (!successful_find)
      {
        ROS_ERROR("couldn't find %dx%d modified circle target in %s, found only %d pts", pattern_rows_, pattern_cols_,
                  image_topic_.c_str(), (int)observation_pts_.size());
      }
      else
      {
        ...
        if (!use_circle_detector_)
          blob_detector_ptr_->detect(image_roi_, keypoints);
        ...
        for (int i = 0; i < (int)keypoints.size(); i++)
        {
          ...
          if (x == centers[start_last_row].x && y == centers[start_last_row].y)
            start_last_row_size = ksize;
          if (x == centers[end_last_row].x && y == centers[end_last_row].y)
            end_last_row_size = ksize;
          if (x == centers[start_1st_row].x && y == centers[start_1st_row].y)
            start_1st_row_size = ksize;
          if (x == centers[end_1st_row].x && y == centers[end_1st_row].y)
            end_1st_row_size = ksize;
        }
      ...
      // TODO: Get point cloud and convert all observations into pt(x,y,z)

      if (successful_find)
      {
        camera_obs_.resize(observation_pts_.size());
        boost::shared_ptr<sensor_msgs::PointCloud2 const> msg;
        msg = ros::topic::waitForMessage<sensor_msgs::PointCloud2>("point_cloud", ros::Duration(2));
        ...
        if (msg)
        {
          ...
          // for all observations, convert from (image_x, image_y) to pt(x,y,z)
          for (int i = 0; i < observation_pts_.size(); ++i)
          {
            // the x,y location of the center of the region is found with sub-pixel accuracy
            // therefore the position in x,y,z of this point in space must be interpolated between the four surrounding
            // point cloud points
            double image_x = observation_pts_[i].x;
            double image_y = observation_pts_[i].y;
            unsigned int fx = floor(image_x);
            unsigned int fy = floor(image_y);
            unsigned int cx = ceil(image_x);
            unsigned int cy = ceil(image_y);
            int height = cloud->height;  // here we are assuming the height and width of the image and the point cloud
                                         // are identical
            ...
            pcl::PointXYZ pt1(pff.x + pctx * (pfc.x - pff.x), pff.y + pctx * (pfc.y - pff.y),
                              pff.z + pctx * (pfc.z - pff.z));  // interpolate along x with y low
            pcl::PointXYZ pt2(pfc.x + pctx * (pcf.x - pcc.x), pfc.y + pctx * (pcf.y - pcc.y),
                              pfc.z + pctx * (pcf.z - pcc.z));  // interpolate along x with y low
            pcl::PointXYZ pt3(pt1.x + pcty * (pt2.x - pt1.x), pt1.y + pcty * (pt2.y - pt1.y),
                              pt1.z + pcty * (pt2.z - pt1.z));  // interp along y between p1&p2

            ROS_INFO("image(%f %f) pff(%f %f %f), tpt(%f %f %f)", image_x, image_y, pff.x, pff.y, pff.z, pt3.x, pt3.y, pt3.z);

            // done getting point from cloud, populate camera observations to return
            camera_obs_.at(i).target = instance_target_;
            camera_obs_.at(i).point_id = i;
            camera_obs_.at(i).image_loc_x = pt3.x;
            camera_obs_.at(i).image_loc_y = pt3.y;
            camera_obs_.at(i).image_loc_z = pt3.z;
            camera_obs_.at(i).cost_type = cost_type_;

          }  // end for loop over all observations

          // return observations
          cam_obs = camera_obs_;
          return successful_find;

        }  // end check for point cloud
        else
        {
          ROS_WARN("did not receive point cloud, not transforming points");
        }
      }  // end if(successful_find)
    }
    ...
    {  // needed to contain scope of automatic variables to this case
      ...
      std::vector<cv::KeyPoint> keypoints;
      if (!use_circle_detector_)
        blob_detector_ptr_->detect(red_binary_image, keypoints);
      if (use_circle_detector_)
      ...
      if (!use_circle_detector_)
        blob_detector_ptr_->detect(green_binary_image, keypoints);
      if (use_circle_detector_)
      ...
      if (!use_circle_detector_)
        blob_detector_ptr_->detect(yellow_binary_image, keypoints);
      if (use_circle_detector_)
      ...
      if (observation_pts_.size() != 3)
      {
        ...
        if (debug_red | debug_green | debug_yellow)
  ...
  else
  {
    ...
    if (!sym_circle_)
      ROS_ERROR("not a symetric target????");
    cv::Point p;
...
void ROSCameraObserver::triggerCamera()
{
  ...
  else
  {
    ...
    while (!done)
    {
      ...
      try
      {
        if (recent_image->encoding == "mono16")
        {  // asus and kinect ir images are mono16, bridge mishandles conversion to mono8
