...
shared_ptr<Camera> parseSingleCamera(const Node &node)
{
  ...
  try
  {
    ...
    if (!parseString(node, "camera_name", temp_name))
      ROS_ERROR("Can't read camera_name");
    if (!parseString(node, "trigger", trigger_name))
      ROS_ERROR("Can't read trigger");
    if (!parseString(node, "image_topic", temp_topic))
      ROS_ERROR("Can't read image_topic");
    if (!parseString(node, "camera_optical_frame", camera_optical_frame))
      ROS_ERROR("Can't read camera_optical_frame");
    if (!parseString(node, "transform_interface", transform_interface))
      ROS_ERROR("Can't read transform_interface");
    ...
    if (!parseDouble(node, "focal_length_x", temp_parameters.focal_length_x))
      ROS_ERROR("Can't read focal_length_x");
    if (!parseDouble(node, "focal_length_y", temp_parameters.focal_length_y))
      ROS_ERROR("Can't read focal_length_y");
    if (!parseDouble(node, "center_x", temp_parameters.center_x))
      ROS_ERROR("Can't read center_x");
    if (!parseDouble(node, "center_y", temp_parameters.center_y))
      ROS_ERROR("Can't read center_y");
    if (!parseDouble(node, "distortion_k1", temp_parameters.distortion_k1))
      ROS_ERROR("Can't read distortion_k1");
    if (!parseDouble(node, "distortion_k2", temp_parameters.distortion_k2))
      ROS_ERROR("Can't read distortion_k2");
    if (!parseDouble(node, "distortion_k3", temp_parameters.distortion_k3))
      ROS_ERROR("Can't read distortion_k2");
    if (!parseDouble(node, "distortion_p1", temp_parameters.distortion_p1))
      ROS_ERROR("Can't read distortion_p1");
    if (!parseDouble(node, "distortion_p2", temp_parameters.distortion_p2))
      ROS_ERROR("Can't read distortion_p2");
    if (!parseInt(node, "image_height", temp_parameters.height))
      ROS_ERROR("Can't read image_height");
    if (!parseInt(node, "image_width", temp_parameters.width))
      ROS_ERROR("Can't read image_width");
...
shared_ptr<Trigger> parseTrigger(const Node &node, string &name)
{
  ...
  {
    if (!parseString(node, "trig_param", trig_param))
      ROS_ERROR("Can't read trig_param");
    temp_trigger = boost::make_shared<ROSParamTrigger>(trig_param);
  }
  else if (name == string("ROS_ACTION_TRIGGER"))
  {
    if (!parseString(node, "trig_action_server", trig_action_server))
      ROS_ERROR("Can't read trig_action_server");
    if (!parseString(node, "trig_action_msg", trig_action_msg))
      ROS_ERROR("Can't read trig_action_msg");
  ...
  else if (name == string("ROS_ROBOT_JOINT_VALUES_ACTION_TRIGGER"))
  {
    ...
    if (!parseVectorD(node, "joint_values", joint_values))
  ...
  else if (name == string("ROS_CAMERA_OBSERVER_TRIGGER"))
  {
    ...
    if (!parseString(trig_node[0], "service_name", service_name))
      ROS_ERROR("Can't read service_name");
    if (!parseString(trig_node[0], "instructions", instructions))
      ROS_ERROR("Can't read instructions");
    if (!parseString(trig_node[0], "image_topic", image_topic))
      ROS_ERROR("Can't read image_topic");
    if (!parseInt(trig_node[0], "roi_min_x", roi.x_min))
      ROS_ERROR("Can't read roi_min_x");
    if (!parseInt(trig_node[0], "roi_max_x", roi.x_max))
      ROS_ERROR("Can't read roi_max_x");
    if (!parseInt(trig_node[0], "roi_min_y", roi.y_min))
      ROS_ERROR("Can't read roi_min_y");
    if (!parseInt(trig_node[0], "roi_max_y", roi.y_max))
      ROS_ERROR("Can't read roi_max_y");
                                                       ...
                                                       Pose6d &pose)
{
  ...
  {
    if (!parseString(node, "camera_housing_frame", camera_housing_frame))
      ROS_ERROR("Can't read camera_housing_frame");
  ...
  {
    if (!parseString(node, "parent_frame", camera_mounting_frame))
      ROS_ERROR("Can't read parent_frame");
